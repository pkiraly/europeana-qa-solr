...
define('SET_FILE_NAME', 'uniqueness-setlist.txt');
...
$i = 1;
while (time() < $endTime) {
  $threads = exec('ps aux | grep "[ ]incremental-index-uniqueness.php" | wc -l');
...
function launch_threads($running_threads) {
  ...
  if (filesize(SET_FILE_NAME) > 3 && isSolrAvailable()) {
    ...
    foreach ($files as $file) {
      ...
      exec('nohup php incremental-index-uniqueness.php ' . $file . ' >>index-report.log 2>>index-report.log &');
    }
  }
}
