...
index_file(){
  ...
  php incremental-index-multilingual-saturation.php --port 8984 --collection $VERSION --with-check --file $FILE
}

get_running_tasks_count(){
  task_count=$(ps aux | grep "[ ]incremental-index-multilingual-saturation.php" | wc -l)
}
...
files=$(ls $dir/parts-multilingual-saturation/*.csv)
...
time=$(date +"%F %T")
echo "$time> index-multilingual-saturation DONE"
